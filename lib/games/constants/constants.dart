
 /* Think before making changes */
 // Those with "Af" are the routes specified for afghan users
 // Those with "Fa" for all persian users such as afghan, iranian...
 // Without country/language code are for all english speakers

List<String> quizListConstantsEnRoutes = ['Geography', 'English', 'General Knowledge'];
List<String> quizListConstantsAfRoutes = ['geography_fa', 'hazaragi_af', 'herati_af', 'english_fa','pashto_af', 'general_nowledge_fa'];
List<String> quizListConstantsFa = ['جغرافیا', 'هزارگی', 'هراتی', 'انگلیسی', 'پشتو','اطلاعات عمومی'];

List<String> gameListConstants = ['Quiz', 'Ludo',];
List<String> gameListConstantsFa = ['مسابقه آزمونی', 'لودو'];

List<String> ludoTypes = ['Online', 'Robots'];


